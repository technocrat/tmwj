using Accessors                # ↪ gives you @modify (and @optic, @set, …)
using Unitful                  # for u"km" etc. must be before MakieExtra
...
function Makie.plot!(p::Scalebar)
    ...
    obs = @lift let
        ...
        mul = argmin(m -> abs(1 / units_in_data * m - $(p.target_ax_frac) * GeometryBasics.width(xlims)), $(p.muls))
        length_data = 1 / units_in_data * mul

        length_ax = length_data / GeometryBasics.width(xlims)
