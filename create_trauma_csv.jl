...
url = "https://en.wikipedia.org/wiki/List_of_trauma_centers_in_the_United_States"
df = extract_nth_wikipedia_table(url)
level1 = subset(df,5 => ByRow(x -> x == "I"))
select!(level1,Cols(1:3))
level1.loc = level1.City .* ", " .* level1.State
df = unique!(level1.loc)